...
export XCIND_VERSION="0.6.0"

__XCIND_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# --------------------------------------------------------------------------
# Build provenance (optional, per-install)
# --------------------------------------------------------------------------
#
# xcind-build-info.bash is generated by the distribution channel (Nix flake,
# install.sh, Dockerfile). It declares XCIND_BUILD_* variables that describe
# how this particular copy was built. Absent for tagged releases via npm, and
# for source trees that are not a git checkout.

if [ -f "$__XCIND_LIB_DIR/xcind-build-info.bash" ]; then
...
# Print the version string with optional build-metadata suffix.
#
# Format: <XCIND_VERSION>[+<SOURCE>[.<SHORT_REV>][.dirty][.<DATE>]]
# where <DATE> is a compact YYYYMMDD derived from the ISO 8601
# XCIND_BUILD_DATE. The `+` suffix follows SemVer build-metadata rules and
# is ignored for version comparison.
#
# Usage:
#   __xcind-version-string        → "0.5.0" or "0.5.0+nix.1a2b3c4.20260420"
__xcind-version-string() {
  local v="$XCIND_VERSION"
  if [ -n "$XCIND_BUILD_SOURCE" ]; then
    local suffix="$XCIND_BUILD_SOURCE"
    if [ -n "$XCIND_BUILD_SHORT_REV" ]; then
      suffix="$suffix.$XCIND_BUILD_SHORT_REV"
    fi
    if [ "$XCIND_BUILD_DIRTY" = "1" ]; then
      suffix="$suffix.dirty"
    fi
    if [ -n "$XCIND_BUILD_DATE" ]; then
      local compact_date="${XCIND_BUILD_DATE:0:10}"
      compact_date="${compact_date//-/}"
      if [ -n "$compact_date" ]; then
        suffix="$suffix.$compact_date"
      fi
    fi
    v="$v+$suffix"
  fi
...
__xcind-check-deps() {
  ...
  echo "xcind v$(__xcind-version-string) — dependency check"
  echo ""
...
__xcind-doctor() {
  ...
  *)
  ...
  if [[ $mode == "text" ]]; then
    echo "xcind v$(__xcind-version-string) — doctor"
    echo ""
