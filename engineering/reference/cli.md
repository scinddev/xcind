...
- **Default bin**: when a bin named `__default` is declared, a name that matches nothing above runs on `__default`'s service as `docker compose … exec <svc> [cmd…] <name> <args…>` (`use=run` → `run --rm`). `__default`'s `cmd` defaults to empty and acts as a prefix. Step-level `@refs` inside scripts stay strict. Without `__default`, an unknown name exits 1 with a hint to declare one. A script named `__default` is a load-time error. `xcind-run --list` shows the fallback on a `default:` line; `--list --names` hides it like any `_`-prefixed name.
