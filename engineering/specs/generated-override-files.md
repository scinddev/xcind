...
| `xcind-host-gateway-hook` | `compose.host-gateway.yaml` | Maps `host.docker.internal` via `extra_hosts`; exposes `XCIND_HOST_GATEWAY` via `environment` |
...
Generated by default (unless disabled with `XCIND_HOST_GATEWAY_ENABLED=0`). The overlay carries two halves:

- **`extra_hosts`** — maps `host.docker.internal` to the developer's workstation for every service that doesn't already define the mapping.
- **`environment`** — exposes the same resolved value as `XCIND_HOST_GATEWAY` inside every service that doesn't already set that variable, for tools that read a host address from the environment rather than resolving a DNS name (Xdebug `client_host` is the motivating case).

The target value is auto-detected based on the platform (native Linux, WSL2, Docker Desktop) or can be overridden with `XCIND_HOST_GATEWAY`.
...
Each half is emitted per service and independently: a service that already has a `host.docker.internal` entry in `extra_hosts` (using either `:` or `=` separator) still receives the variable, and a service that already sets `XCIND_HOST_GATEWAY` itself (map or `NAME=value` list form) keeps its own value and still receives the mapping. A service that already has both is omitted; when every service has both, the hook produces no output. Compose merges `environment` maps by key, so — unlike `extra_hosts`, whose existing entries are carried forward — the variable half needs no carry-forward.

On Docker Desktop, where `host.docker.internal` is resolved via DNS automatically, detection returns no value and the hook produces no output at all — so neither half is emitted there.
