...
| `docs/specs/workspace-lifecycle.md` | `engineering/specs/workspace-lifecycle.md` | PARTIAL | P4/P5 | Scind full state machine + flavors + destroy (P5); Xcind deliberately stateless registry (P4). R4: Xcind adds workspace-wide `up`/`down` (`12737f8`, divergence 0028 resolved); Scind-only residue narrows to state machine/flavors/clone/generate + `restart`. R5: `restart` + per-app `-a` filter + `down --volumes` land (#90/#91, RL-152) — orchestration surface fully converged; residue narrows to state machine/flavors/clone/generate. |
...
| `docs/reference/cli.md` | `engineering/reference/cli.md` | PARTIAL | P4/P5 | Scind unified `scind <resource> <action>` vs Xcind six binaries. Scind-only surface (flavor cmds, clone/generate/destroy, port assign/gc/scan) → P5; Xcind-ahead (`xcind-prompt`, provenance `--version`, `--generate-*`, `application ports/urls/exports`) → P4. R2: + `xcind-config resolve <path>`, `dispose` on proxy/workspace/application, external-mode proxy flags. R4: + `xcind-workspace up/down` (`12737f8`) — converges on canon's one-command orchestration (0028 resolved); Scind-only workspace surface narrows to `restart`/`clone`/`generate` + per-app `-a` targeting. R5: workspace `restart`, repeatable `-a` targeting, and `down --volumes` converge (#90/#91, RL-152); Scind-only workspace surface narrows to `clone`/`generate`. |
