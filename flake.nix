{
  ...
  outputs = { self, nixpkgs, flake-utils }:
    let
      ...
      # Convert self.lastModifiedDate (compact "YYYYMMDDhhmmss") to ISO 8601.
      # Returns empty string when unavailable so xcind-build-info degrades
      # gracefully in exotic evaluation contexts.
      formatFlakeDate = d:
        if d == null || d == "" then ""
        else
          let
            yyyy = builtins.substring 0 4 d;
            mm   = builtins.substring 4 2 d;
            dd   = builtins.substring 6 2 d;
            hh   = builtins.substring 8 2 d;
            mi   = builtins.substring 10 2 d;
            ss   = builtins.substring 12 2 d;
          in "${yyyy}-${mm}-${dd}T${hh}:${mi}:${ss}Z";

      # Build-provenance attrs derived from the flake's own `self`. Passed as
      # a plain attribute set to each derivation so postInstall can emit
      # xcind-build-info.bash without re-evaluating flake metadata.
      buildInfo = {
        shortRev = self.shortRev or self.dirtyShortRev or "";
        longRev  = self.rev or self.dirtyRev or "";
        date     = formatFlakeDate (self.lastModifiedDate or "");
        dirty    = if self ? dirtyRev then "1" else "0";
      };

      # postInstall snippet that writes xcind-build-info.bash. Shared between
      # wrapped and unwrapped derivations so both channels report provenance.
      writeBuildInfo = ''
        cat > "$out/lib/xcind/xcind-build-info.bash" <<'BUILDINFO_EOF'
        #!/usr/bin/env bash
        # xcind-build-info.bash — Generated by Nix flake. Do not edit.

        XCIND_BUILD_SOURCE="nix"
        XCIND_BUILD_SHORT_REV="${buildInfo.shortRev}"
        XCIND_BUILD_LONG_REV="${buildInfo.longRev}"
        XCIND_BUILD_REF=""
        XCIND_BUILD_DATE="${buildInfo.date}"
        XCIND_BUILD_DIRTY="${buildInfo.dirty}"
        BUILDINFO_EOF
        chmod 644 "$out/lib/xcind/xcind-build-info.bash"
      '';
        ...
        postInstall =
          ...
          in writeBuildInfo + ''
            ...
            wrapProgram "$out/bin/xcind-config" \
              --prefix PATH : ${fullPath}
            wrapProgram "$out/bin/xcind-prompt" \
              --prefix PATH : ${fullPath}
