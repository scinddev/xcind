...
| **Bin** | A command inside a service container, declared in `XCIND_BINS`, run by `xcind-run` and exposed to IDE integrations. The reserved name `__default` declares the fallback service for unknown `xcind-run` names. |
