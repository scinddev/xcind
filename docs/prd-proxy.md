...
6. __xcind-resolve-url-templates → set XCIND_APP_URL_TEMPLATE, XCIND_ROUTER_TEMPLATE,
...
`XCIND_WORKSPACE_SERVICE_TEMPLATE` is passed through as-is (no workspaceless/workspace selection needed).

Downstream code (e.g., `xcind-proxy-hook`) only references `XCIND_APP_URL_TEMPLATE` and `XCIND_ROUTER_TEMPLATE` — no workspace-mode branching needed. `xcind-workspace-hook` references `XCIND_WORKSPACE_SERVICE_TEMPLATE` directly.
...
### 4.8 Workspace Networking

Workspace networking enables inter-application communication within a workspace via Docker network aliases. While the proxy layer (Section 5) handles external access through Traefik, workspace networking provides a separate internal communication path that does not require a reverse proxy.

#### Network

Each workspace gets a shared Docker network named `{workspace}-internal` (e.g., `dev-internal`). This network is declared as `external: true` in generated compose files and lazily created by the workspace hook on first use.

#### Scope: all services

Every compose service in an application gets a workspace network alias — not just exported services. This differs from the proxy layer, which only operates on services declared in `XCIND_PROXY_EXPORTS`. The rationale: internal communication needs to reach any service (databases, workers, sidecars), not just those with external routes.
...
| Concern | Proxy layer | Workspace networking |
|---------|-------------|---------------------|
| Purpose | External access via Traefik | Inter-app communication within workspace |
| Scope | Exported services only (`XCIND_PROXY_EXPORTS`) | All compose services |
| Name source | Export name (may differ from compose service) | Compose service name |
| Network | `xcind-proxy` (host-wide, shared across workspaces) | `{workspace}-internal` (per-workspace) |
| Output file | `compose.proxy.yaml` | `compose.workspace.yaml` |
| Active when | Always (workspaceless or workspace) | Workspace mode only (`XCIND_WORKSPACELESS=0`) |
...
For app "frontend" in workspace "dev" with compose services "web" and "worker":
...
When invoked during a cache miss, `xcind-workspace-hook`:

1. **Check workspace mode** — read `XCIND_WORKSPACELESS`. If `1`, exit 0 immediately with no output (no overlay generated for standalone apps).
2. **Read pipeline env vars** — `XCIND_APP`, `XCIND_WORKSPACE`, and `XCIND_WORKSPACE_SERVICE_TEMPLATE` from the environment (already set by the pipeline).
3. **Enumerate all services** — read `$XCIND_CACHE_DIR/resolved-config.yaml` to get the complete list of compose services (not just exports).
4. **Ensure network exists** — `docker network create "{workspace}-internal" 2>/dev/null || true`. Lazy and idempotent.
5. **For each service** — generate alias via `__xcind-render-template` using `XCIND_WORKSPACE_SERVICE_TEMPLATE` with `workspace`, `app`, and `service` variables.
6. **Write `compose.workspace.yaml`** to `$XCIND_GENERATED_DIR/`.
7. **Print** `-f $XCIND_GENERATED_DIR/compose.workspace.yaml` to stdout.

**Independence from proxy hook:** The workspace hook and proxy hook produce separate overlay files (`compose.workspace.yaml` and `compose.proxy.yaml`). Docker Compose merges both when invoked. The hooks have no dependencies on each other — either can run alone, and order does not matter.
...
### 12.3 Workspace Override Generation

The generated `compose.workspace.yaml` is built using the same hybrid template approach as the proxy overlay.

**Per-service snippet template:**
...
__xcind-render-template "$XCIND_WORKSPACE_SERVICE_SNIPPET" \
  service "$service_name" \
  network "${XCIND_WORKSPACE}-internal" \
  alias "$alias"
```
...
When `xcind-workspace-hook` is invoked during a cache miss:

1. **Receive `$app_root`** as positional argument from the hook system.
2. **Check workspace mode** — if `XCIND_WORKSPACELESS=1`, exit 0 with no output.
3. **Read pipeline env vars** — `XCIND_APP`, `XCIND_WORKSPACE`, `XCIND_WORKSPACE_SERVICE_TEMPLATE`.
4. **Compute network name** — `"${XCIND_WORKSPACE}-internal"`.
5. **Ensure network exists** — `docker network create "$network" 2>/dev/null || true`.
6. **Parse resolved config** — read `$XCIND_CACHE_DIR/resolved-config.yaml` using `yq` to enumerate all service names.
7. **Initialize output** — start with `services:\n` header.
8. **For each service in resolved config:**
   - a. **Generate alias** via `__xcind-render-template` using `XCIND_WORKSPACE_SERVICE_TEMPLATE`.
   - b. **Render service snippet** via `__xcind-render-template` using `XCIND_WORKSPACE_SERVICE_SNIPPET` with service name, network name, and alias.
   - c. **Append** rendered snippet to output.
9. **Append network footer** — `networks:\n  {network}:\n    external: true`.
10. **Write `compose.workspace.yaml`** to `$XCIND_GENERATED_DIR/`.
11. **Print** `-f $XCIND_GENERATED_DIR/compose.workspace.yaml` to stdout.
...
<xcind repo>/
  ...
  lib/xcind/
    ...
    xcind-workspace-lib.bash # NEW — workspace networking logic
  test/
...
<workspace>/
  ...
  <app>/
    ...
    .xcind/
      ...
      generated/<sha>/
        compose.proxy.yaml
        compose.workspace.yaml  # only in workspace mode
...
<standalone app>/
  ...
  .xcind/
    cache/<sha>/
    generated/<sha>/
      compose.proxy.yaml       # proxy overlay (always)
                                # no compose.workspace.yaml (workspaceless)
  compose.yaml             # user's compose file(s)
```
...
- [ ] Non-exported services still get workspace network aliases.
