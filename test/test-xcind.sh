...
# ======================================================================
echo ""
echo "=== Test: xcind-prompt (smoke) ==="

# Lean end-to-end smoke coverage for the bin/xcind-prompt executable. The full
# matrix (override fixture, every apex scheme/TLS permutation, per-row
# stderr/exit/trailing-newline assertions, a proof that --detect sources no
# config) lives in step-05's dedicated test/test-xcind-prompt.sh; step-05 may
# relocate these cases there.
#
# Each case invokes the REAL binary as a subprocess under a hermetic HOME so
# the machine's global proxy config.sh (which would override XCIND_PROXY_DOMAIN)
# and the user's workspace registry cannot leak in. Test hygiene: the resolved
# identity vars are unset before every invocation (via prompt_clear_env) so the
# binary re-derives them from the fixture directory — __xcind-app-root trusts a
# pre-set XCIND_APP_ROOT and __xcind-resolve-app trusts a pre-set XCIND_APP, so a
# value leaked from an earlier suite test would defeat directory-based detection.

prompt_home=$(mktemp_d)
prompt_err_file="$prompt_home/stderr"
...
# OSC 8 introducer bytes (ESC ] 8 ; ;). Built with printf so the terminal never
# interprets them; matched as raw bytes against captured stdout.
prompt_osc8=$(printf '\033]8;;')

# Drop identity vars an earlier suite section may have exported, so the prompt
# subprocess re-resolves them from its cwd. Call inside each invocation subshell.
prompt_clear_env() {
...
# expected non-zero exits (e.g. --detect outside an app).
run_prompt() {
  local dir="$1"
  shift
  prompt_rc=0
...
# --- workspace app: "<workspace>/<app>", exit 0, stderr empty, no trailing \n
# The '@' sentinel makes the trailing-newline check survive $()'s newline strip:
# a trailing newline would leave "dev/frontend\n@" (≠ "dev/frontend@").
prompt_ws_sentinel=$(
...
assert_eq "xcind-prompt: workspace display + no trailing newline" "dev/frontend@" "$prompt_ws_sentinel"
run_prompt "$prompt_ws_app"
assert_eq "xcind-prompt: workspace display stdout" "dev/frontend" "$prompt_out"
assert_eq "xcind-prompt: workspace display exit 0" "0" "$prompt_rc"
assert_eq "xcind-prompt: workspace display stderr empty" "" "$prompt_err"

# --- workspaceless app: "<app>", exit 0, stderr empty
run_prompt "$prompt_wsless_app"
assert_eq "xcind-prompt: workspaceless display stdout" "acmeapps" "$prompt_out"
assert_eq "xcind-prompt: workspaceless display exit 0" "0" "$prompt_rc"
assert_eq "xcind-prompt: workspaceless display stderr empty" "" "$prompt_err"

# --- outside any app: empty stdout, exit 0, stderr empty
prompt_outside=$(mktemp_d)
run_prompt "$prompt_outside"
assert_eq "xcind-prompt: outside stdout empty" "" "$prompt_out"
assert_eq "xcind-prompt: outside exit 0" "0" "$prompt_rc"
assert_eq "xcind-prompt: outside stderr empty" "" "$prompt_err"

# --- --detect in app: empty stdout, exit 0, stderr empty
run_prompt "$prompt_ws_app" --detect
assert_eq "xcind-prompt: --detect in-app stdout empty" "" "$prompt_out"
assert_eq "xcind-prompt: --detect in-app exit 0" "0" "$prompt_rc"
assert_eq "xcind-prompt: --detect in-app stderr empty" "" "$prompt_err"

# --- --detect from a subdirectory: empty stdout, exit 0
run_prompt "$prompt_ws_sub" --detect
assert_eq "xcind-prompt: --detect subdir stdout empty" "" "$prompt_out"
assert_eq "xcind-prompt: --detect subdir exit 0" "0" "$prompt_rc"

# --- --detect outside: empty stdout, non-zero exit, stderr empty
run_prompt "$prompt_outside" --detect
assert_eq "xcind-prompt: --detect outside stdout empty" "" "$prompt_out"
assert_not_contains "xcind-prompt: --detect outside non-zero exit" "0" "$prompt_rc"
assert_eq "xcind-prompt: --detect outside stderr empty" "" "$prompt_err"

# --- --apex (apex present): hostname substring + OSC 8 intro bytes.
# OQ4: assert the hostname substring + OSC 8 byte presence only; do NOT pin the
# scheme (http/https) — that is exhaustively covered in step-05.
run_prompt "$prompt_ws_app" --apex
assert_contains "xcind-prompt: --apex contains apex hostname" "dev-frontend.xcind.localhost" "$prompt_out"
assert_contains "xcind-prompt: --apex emits OSC 8 intro bytes" "$prompt_osc8" "$prompt_out"

# --- --apex --no-hyperlink: bare hostname, no OSC 8 bytes
run_prompt "$prompt_ws_app" --apex --no-hyperlink
assert_contains "xcind-prompt: --no-hyperlink contains bare hostname" "dev-frontend.xcind.localhost" "$prompt_out"
assert_not_contains "xcind-prompt: --no-hyperlink omits OSC 8 bytes" "$prompt_osc8" "$prompt_out"

# --- XCIND_PROMPT_HYPERLINKS=0 --apex: equivalent to --no-hyperlink
prompt_rc=0
prompt_out=$(
  cd "$prompt_ws_app" || exit 99
  prompt_clear_env
  HOME="$prompt_home" XDG_STATE_HOME="$prompt_home/state" \
    XDG_CONFIG_HOME="$prompt_home/config" XCIND_PROMPT_HYPERLINKS=0 \
    "$XCIND_ROOT/bin/xcind-prompt" --apex 2>"$prompt_err_file"
) || prompt_rc=$?
assert_contains "xcind-prompt: XCIND_PROMPT_HYPERLINKS=0 contains bare hostname" "dev-frontend.xcind.localhost" "$prompt_out"
assert_not_contains "xcind-prompt: XCIND_PROMPT_HYPERLINKS=0 omits OSC 8 bytes" "$prompt_osc8" "$prompt_out"

# --- --apex on an app with no proxied export: display only (no apex appended)
run_prompt "$prompt_wsless_app" --apex
assert_eq "xcind-prompt: --apex with no apex yields display only" "acmeapps" "$prompt_out"

# --- registry no-write: a fresh hermetic HOME must contain no registry file
# after a workspace run (XCIND_NO_REGISTRY=1 is set before discovery).
prompt_reg_home=$(mktemp_d)
(
  cd "$prompt_ws_app" || exit 99
  prompt_clear_env
  HOME="$prompt_reg_home" XDG_STATE_HOME="$prompt_reg_home/state" \
    XDG_CONFIG_HOME="$prompt_reg_home/config" \
    "$XCIND_ROOT/bin/xcind-prompt" >/dev/null 2>&1
) || true
assert_file_missing "xcind-prompt: performs no workspace registry write" "$prompt_reg_home/state/xcind/workspaces.tsv"
